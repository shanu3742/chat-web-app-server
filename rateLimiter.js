...
//for signin only 5 times a user can try to do login with in 10 minutes
let resetPasswordLimiter = rateLimit({
  max: APP_CONFIG.RESET_PASSWORD_MAX_ATTEMPTS,
  windowMs: APP_CONFIG.RESET_PASSWORD_WINDOW_DURATION,
  message: {
    message: APP_CONFIG.RESET_RATE_LIMIT_MESSAGE,
  },
  standardHeaders: true, // Send rate limit info in the `RateLimit-*` headers
  legacyHeaders: false, // Disable the `X-RateLimit-*` headers
});

module.exports = { limiter, signinLimiter, resetPasswordLimiter };
